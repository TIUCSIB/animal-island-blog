...
export function AdminStickerPanel({ onSelect }: AdminStickerPanelProps) {
  const [page, setPage] = useState(0)
  const lastWheelAtRef = useRef(0)
  const pages = useMemo(() => {
    const stickers = stickerSource.map((item) => {
      const [alt, src] = item.split('|')
      return { alt, src }
    })
    ...
    <section className="island-admin-sticker-panel">
      <div className="island-admin-sticker-panel__viewport" onWheel={handleWheel}>
        <div className="island-admin-sticker-panel__clip">
          <div className="island-admin-sticker-panel__track" style={{ transform: `translate3d(-${page * pageWidth}px, 0, 0)` }}>
            {pages.map((stickers, index) => (
              <ul key={index} className="island-admin-sticker-panel__page">
                {stickers.map((sticker) => (
                  <li key={sticker.src}>
                    <button type="button" title={sticker.alt} aria-label={sticker.alt} onClick={() => onSelect(sticker)}>
